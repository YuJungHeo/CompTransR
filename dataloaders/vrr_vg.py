...
def make_cluster_indices(n_boxes, max_batch_size, mode, minieval=False, validate_batch_size=False):
    ...
    bins = np.arange(5, 210, 5)
    if mode == 'val' or mode == 'test':
        num_batch = [100, 50, 20, 10, 5, 5, 3, 3] + [1]*(len(bins)-8)  # 'nu2_eu3_lr00001_sg_ns'
        
        num_batch = [int(x * max_batch_size / 100) for x in num_batch]  # num_batch >= 1
    else:
        num_batch = [100, 50, 20, 10, 5, 5, 3, 3] + [1]*(len(bins)-8)
        num_batch = [int(x * max_batch_size / 100) for x in num_batch]  # num_batch >= 1
...
def load_graphs_vrrvg(mode, dpath):
    gfile = 'VrR-VG-{tier}_scenegraphs.pkl'.format(tier=mode)
