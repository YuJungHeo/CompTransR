"""
This code is based on Rowan's repository.
...
from units.pytorch_misc import intersect_2d, argsort_desc
from units.box_intersections_cpu.bbox import bbox_overlaps
...
class BasicSceneGraphEvaluator:
    ...
    def print_stats(self):
        print('======================' + self.mode + ' @ ' + epoch + ' ============================')
        for k, v in self.result_dict[self.mode + '_recall'].items():
            print('R@%i: %f' % (k, np.mean(v)))

    def print_stats_log(self, logger, writer, epoch):
        logger.write('======================' + self.mode + '============================')
        for k, v in self.result_dict[self.mode + '_recall'].items():
            logger.write('R@%i: %f' % (k, np.mean(v)))
            writer.add_scalar('mtr/R@%i'%(k), np.mean(v), epoch)
...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, multiple_preds=False,
    ...
    rel_inds = pred_entry['rel_inds']
    rel_scores = pred_entry['rel_scores']

    if mode == 'predcls' or mode == 'preddet':
        pred_boxes = gt_boxes
    ...
    if multiple_preds:
        ...
        """original rel_scores: [k, 51] -> current rel_scores: [k]
                # pred_rels = np.column_stack((pred_rel_inds, 1+rel_scores[:,1:].argmax(1)))
                # predicate_scores = rel_scores[:,1:].max(1)
        """
        pred_rels = np.column_stack((pred_rel_inds, rel_inds))
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
    ...
    # Exclude self rels
    # assert np.all(pred_rels[:,0] != pred_rels[:,1]) # if #objs is too small, self rels are captured
...
def _triplet(predicates, relations, classes, boxes,
    ...
    if predicate_scores is not None and class_scores is not None:
        triplet_scores = np.column_stack((
            ...
            predicate_scores,#.cpu().detach().numpy(),
        ))
